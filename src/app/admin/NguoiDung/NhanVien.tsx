...
import ThemNhanVien from "./ThemNguoiDung.jsx";
import ChiTietNguoiDung from "./ChiTietNguoiDung.jsx";
import CapNhatNguoiDung from "./CapNhatNguoiDung.jsx";
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "http://localhost:8080/api/nguoidung?vaiTroId=1"
        );
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmDelete) {
      try {
        ...
        message.error("Không thể xóa người dùng."); // Show error message
      }
    }
  };
